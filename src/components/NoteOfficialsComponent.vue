<template>
  <scroller lock-x scrollbar-y use-pullup use-pulldown ref="noteListScroller" :height="(clientHeight - 50) +'px'" @on-pulldown-loading="onPulldown" @on-pullup-loading="onPullup">
    <note-list-component :name="name" :tops="{notes: topData.notes, userExts: topData.userExts}" :bodys="{notes: bodyData.notes, userExts: bodyData.userExts}"></note-list-component>
  </scroller>
</template>
  ...
  export default {
    ...
    methods: {
      ...
      requestData(callback) {
        Util.ajax.get('/note/find_official_notes/' + this.$route.params.id + '/' + this.bodyData.page.pageNextId + '/' + this.bodyData.page.pageSize + '/').then((response) => {
